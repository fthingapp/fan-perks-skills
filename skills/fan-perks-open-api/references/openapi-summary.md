...
| GET | `/withdraw/config` | `withdraw:read` | on | 100/day | Withdraw config, balance, methods, and Open API limits |
...
`POST /withdraw/apply` requires:

```http
Idempotency-Key: withdraw-20260613-001
```

Rules:

- Generate a unique key for each intended withdraw application.
- Reusing the same key should return the first result and may include `idempotent_replay: true`.
- Before applying, call `/withdraw/config` and verify `open_api_withdraw_apply_enabled`, `open_api_limits.daily_count`, `open_api_limits.daily_amount`, `open_api_limits.single_max_amount`, minimum amount, fee, and available balance.
...
| `SERVER_ERROR` | 200 | Server-side error. | Retry only safe reads; preserve idempotency key for withdraw. |
...
curl -H "Authorization: Bearer $FAN_PERKS_API_KEY" \
  "$FAN_PERKS_BASE_URL/open/tkcps/v1/me"
```
...
```bash
curl -H "Authorization: Bearer $FAN_PERKS_API_KEY" \
  "$FAN_PERKS_BASE_URL/open/tkcps/v1/goods/search?keyword=%E5%95%86%E5%93%81%E9%93%BE%E6%8E%A5"
```

Convert a product link:

```bash
curl -X POST "$FAN_PERKS_BASE_URL/open/tkcps/v1/goods/convert" \
...
```bash
curl -X POST "$FAN_PERKS_BASE_URL/open/tkcps/v1/withdraw/apply" \
  -H "Authorization: Bearer $FAN_PERKS_API_KEY" \
