...
| Skill | 说明 |
| --- | --- |
| `fan-perks-open-api` | 搜索商品优惠、转换返利链接、读取会员 CPS 订单、读取会员红包/积分/等级、提交提现申请、查询提现记录。 |
...
商品操作顺序：搜索商品 → 选择商品 → 使用 product_ref 查询优惠。搜索不再生成推广链接；预估红包可以直接展示，缺失时标记待查询。推广结果按会员隔离复用 10 分钟，商品引用有效 24 小时。
